...
func _prepare_identity() -> void:
	...
	_game_state.set("dyad_id", "DATA_VALIDATION")
	_game_state.set("participant_A", "A001")
	_game_state.set("participant_B", "B001")
	_game_state.set("relation_condition", "friends")
	_game_state.set("experiment_condition", "perturbation")
...
func _assert_derived_outputs(summary: Dictionary) -> void:
	var directory: String = str(summary.get("analysis_directory", summary["data_directory"]))
	for filename: String in [
		"analysis_metadata.csv", "trial_summary.csv", "perturbation_summary.csv",
		"dyad_summary.csv", "review_queue.csv", "review_windows.csv",
		"review_agreement.csv",
	]:
		_expect(FileAccess.file_exists(directory.path_join(filename)), "%s missing" % filename)
	var queue: Array[Dictionary] = _read_csv_checked(directory.path_join("review_queue.csv"))
	_expect(not queue.is_empty(), "review queue did not sample the perturbation")
	if not queue.is_empty():
		var svg_path: String = directory.path_join(str(queue[0].get("svg_path", "")))
		_expect(FileAccess.file_exists(svg_path), "automatic review SVG missing")
		var svg: FileAccess = FileAccess.open(svg_path, FileAccess.READ)
		_expect(svg != null and svg.get_as_text().contains("<svg"), "review SVG is unreadable")
...
func _test_interruption_recovery() -> void:
	...
	if user_dir == null:
		return
	user_dir.make_dir_recursive("data_validation_recovery/CASE")
	...
	_write_fixture_csv(
		RECOVERY_SESSION.path_join("session.csv"),
		session_columns,
		{"schema_version": schema["schema_version"], "session_id": "RECOVER_CASE", "force_max": 1.0},
	)
	_write_fixture_csv(RECOVERY_SESSION.path_join("frames.csv"), frame_columns, {})
	_write_fixture_csv(
		RECOVERY_SESSION.path_join("events.csv"),
		event_columns,
		{
			...
			"experiment_condition": "baseline", "event_type": "run_start",
		},
	)
	...
	_expect(
		FileAccess.file_exists(RECOVERY_SESSION.path_join("trial_summary.csv")),
		"recovery did not regenerate derived CSV",
	)
